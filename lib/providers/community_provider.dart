...
/// Community warnings notifier for managing warning operations
class CommunityWarningsNotifier extends StateNotifier<AsyncValue<List<CommunityWarning>>> {
  final FirebaseService _firebaseService;
  ...
  Future<List<CommunityWarning>> _getWarningsFromFirestore() async {
    final snapshot = await _firebaseService.getNearbyWarnings(0, 0, 999999).first;
    return snapshot.docs
        .map((doc) => CommunityWarning.fromMap({
          'id': doc.id,
          ...doc.data() as Map<String, dynamic>,
  ...
  Future<void> submitWarning(CommunityWarning warning) async {
    ...
    try {
      ...
      debugService.logAction(
        action: 'Warning: Starting Firebase submitWarning call',
        screen: 'CommunityWarningsNotifier',
        parameters: {
          'warningTitle': warning.title,
          'warningType': warning.type,
          'warningData': warning.toMap(),
        },
      );
      
      // Call Firebase to add the warning
      await _firebaseService.submitWarning(warning.toMap());
      
      debugService.logAction(
        action: 'Warning: Successfully added to Firebase',
        screen: 'CommunityWarningsNotifier',
        result: 'Warning created in Firestore',
      );
      
      // Reload warnings after successful creation
      final warnings = await _getWarningsFromFirestore();
      state = AsyncValue.data(warnings);
      
      debugService.logAction(
        action: 'Warning: Reloaded warnings after creation',
        screen: 'CommunityWarningsNotifier',
        parameters: {'warningCount': warnings.length},
      );
    } catch (error, stackTrace) {
      debugService.logAction(
        action: 'Warning: Failed to add to Firebase',
        screen: 'CommunityWarningsNotifier',
        error: error.toString(),
      );
  ...
  Future<void> updateWarning(String documentId, CommunityWarning warning) async {
    ...
    try {
      ...
      debugService.logAction(
        action: 'Warning: Starting Firebase updateWarning call',
        screen: 'CommunityWarningsNotifier',
        parameters: {
          'warningId': documentId,
          'warningTitle': warning.title,
          'warningType': warning.type,
        },
      );
      
      // Call Firebase to update the warning
      await _firebaseService.updateWarning(documentId, warning.toMap());
      
      debugService.logAction(
        action: 'Warning: Successfully updated in Firebase',
        screen: 'CommunityWarningsNotifier',
        result: 'Warning updated in Firestore',
      );
      ...
      debugService.logAction(
        action: 'Warning: Reloaded warnings after update',
        screen: 'CommunityWarningsNotifier',
        parameters: {'warningCount': warnings.length},
      );
    } catch (error, stackTrace) {
      debugService.logAction(
        action: 'Warning: Failed to update in Firebase',
        screen: 'CommunityWarningsNotifier',
        error: error.toString(),
      );
  ...
  Future<void> deleteWarning(String warningId) async {
    ...
    try {
      ...
      debugService.logAction(
        action: 'Warning: Starting Firebase deleteWarning call',
        screen: 'CommunityWarningsNotifier',
        parameters: {'warningId': warningId},
      );
      
      // Call Firebase to delete the warning
      await _firebaseService.deleteWarning(warningId);
      
      debugService.logAction(
        action: 'Warning: Successfully deleted from Firebase',
        screen: 'CommunityWarningsNotifier',
        result: 'Warning deleted from Firestore',
      );
      
      // Reload warnings after successful deletion
      final warnings = await _getWarningsFromFirestore();
      state = AsyncValue.data(warnings);
      
      debugService.logAction(
        action: 'Warning: Reloaded warnings after deletion',
        screen: 'CommunityWarningsNotifier',
        parameters: {'warningCount': warnings.length},
      );
    } catch (error, stackTrace) {
      debugService.logAction(
        action: 'Warning: Failed to delete from Firebase',
        screen: 'CommunityWarningsNotifier',
        error: error.toString(),
      );
